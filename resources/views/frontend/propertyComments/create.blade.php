        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.propertyComment.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.property-comments.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            @foreach(App\Models\PropertyComment::RATE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="rate_{{ $key }}" name="rate" value="{{ $key }}" {{ old('rate', '') === (string) $key ? 'checked' : '' }} required>
