    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.bookings.update", [$booking->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id">
                    @foreach($customers as $id => $customer)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $booking->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $customer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('property') ? 'is-invalid' : '' }}" name="property_id" id="property_id" required>
                    @foreach($properties as $id => $property)
                        <option value="{{ $id }}" {{ (old('property_id') ? old('property_id') : $booking->property->id ?? '') == $id ? 'selected' : '' }}>{{ $property }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $booking->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Booking::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $booking->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paid_amount') ? 'is-invalid' : '' }}" type="number" name="paid_amount" id="paid_amount" value="{{ old('paid_amount', $booking->paid_amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payable_amount') ? 'is-invalid' : '' }}" type="number" name="payable_amount" id="payable_amount" value="{{ old('payable_amount', $booking->payable_amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('booking_form') ? 'is-invalid' : '' }}" type="text" name="booking_form" id="booking_form" value="{{ old('booking_form', $booking->booking_form) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('booking_until') ? 'is-invalid' : '' }}" type="text" name="booking_until" id="booking_until" value="{{ old('booking_until', $booking->booking_until) }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Booking::BOOKING_STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('booking_status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="booking_status_{{ $key }}" name="booking_status" value="{{ $key }}" {{ old('booking_status', $booking->booking_status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('document_number') ? 'is-invalid' : '' }}" type="text" name="document_number" id="document_number" value="{{ old('document_number', $booking->document_number) }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('document_verified') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="document_verified" id="document_verified" value="1" {{ $booking->document_verified || old('document_verified', 0) === 1 ? 'checked' : '' }}>
